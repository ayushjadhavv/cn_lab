def water_jug_problem_all_solutions(jug1_capacity, jug2_capacity, target):
    stack = []
    visited = set()
    solutions = []
    stack.append(((0, 0), [(0, 0)], []))
    print("\nStarting Water Jug Problem...")
    print("Initial state: (0, 0)")
    while stack:
        (jug1, jug2), path, operations = stack.pop()
        ...
        for state, operation in next_states:
            if state not in visited:
                stack.append((state, path + [state], operations + [operation]))
    return solutions

jug1_capacity = int(input("Enter the capacity of Jug 1: "))
jug2_capacity = int(input("Enter the capacity of Jug 2: "))
target = int(input("Enter the target amount: "))
solutions = water_jug_problem_all_solutions(jug1_capacity, jug2_capacity, target)
if solutions:
